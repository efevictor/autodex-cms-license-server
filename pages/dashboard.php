...
$version_dist          = ls_rows("
    SELECT COALESCE(NULLIF(cms_version,''), '(unknown)') AS version, COUNT(*) AS installs
    FROM licenses
    WHERE status = 'active' AND activated_domain IS NOT NULL
    GROUP BY cms_version ORDER BY installs DESC
");
...
<div class="ls-card mb-4">
    <div class="ls-card-hd"><h6>Products</h6></div>
    <div class="table-responsive">
        <table class="ls-table">
            <thead>
                <tr><th>Product</th><th>Total Licenses</th><th>Active</th><th>Activated</th></tr>
            </thead>
            <tbody>
            ...
            <tr>
                <td>
                    ...
                    <div class="text-muted" style="font-size:.75rem"><?= e($prod['slug']) ?></div>
                </td>
                        ...
                        <tr>
                            <td>
                                <a href="<?= ls_url('licenses.view', ['id' => $l['id']]) ?>"
                                   class="lic-key text-decoration-none ls-tooltip"
                                   data-tip="<?= e($l['product_name']) ?>">
                                    <?= e($l['license_key']) ?>
                                </a>
                            </td>
                            <td class="text-muted small"><?= e($l['purchase_email']) ?></td>
                            <td class="text-muted small"><?= $l['activated_domain'] ? e($l['activated_domain']) : '—' ?></td>
    ...
    <div class="col-lg-5">
        <div class="ls-card">
            ...
            <div class="d-flex align-items-start gap-2 px-3 py-2 border-bottom" style="border-color:#f0f2f5 !important">
                ...
                <span class="ms-auto text-muted" style="white-space:nowrap;font-size:.65rem;padding-top:2px"><?= e($a['event']) ?></span>
            </div>
            <?php endforeach; endif; ?>
        </div>
    </div>
...
<div class="ls-card">
    ...
    <div class="p-4">
        ...
        <div class="d-flex gap-4 flex-wrap mb-4" style="font-size:.85rem">
            <div><span class="badge-active me-1"><?= $up_to_date_count ?></span><span class="text-muted">Up to date</span></div>
            <?php if ($outdated_count > 0): ?>
            <div><span class="badge-suspended me-1"><?= $outdated_count ?></span><span class="text-muted">Outdated</span></div>
            <?php endif; ?>
            <?php if ($unknown_count > 0): ?>
            <div><span class="badge bg-light text-muted border me-1"><?= $unknown_count ?></span><span class="text-muted">Not yet reported</span></div>
            <?php endif; ?>
        </div>

        <?php if (!empty($version_dist)): ?>
        <table class="ls-table">
            <thead><tr><th>Installed Version</th><th>Installs</th><th>Status</th></tr></thead>
            <tbody>
...
.ls-tooltip:hover::after { opacity: 1; }
</style>
